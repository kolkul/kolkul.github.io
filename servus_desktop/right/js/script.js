...
$(window).on('mousemove', function(event) {
	if (Math.round(oldClientX)!=Math.round(event.clientX)) {
		...
		var obj={mousePos: event.clientX+iFrameWidth[0]+iFrameWidth[1]};
		if (moveSlider) {
	...
	if (typeof(event.data) !== 'undefined') {
		try {
			...
			if (typeof obj.mousePos !== 'undefined' && moveSlider) {
				send_size=false;
				var mpos=obj.mousePos-iFrameWidth[0]-iFrameWidth[1];
				$('.page-container').trigger('twentytwenty_move',[mpos]);
				// $(debug).html(mpos);
			}

			if (send_size) {
				obj={elWidth: window.innerWidth, elNum: 2};
				parent.postMessage(JSON.stringify(obj), '*');
			}
		} catch (e) {}
	}
